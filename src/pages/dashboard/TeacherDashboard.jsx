...
export default function TeacherDashboard() {
  ...
  const isFormComplete = !!(newSubName?.trim() && newSubCode?.trim() && newSubType && newSubBatch);
  ...
  // ✅ Sahi handleCreateSubject - koi duplicate nahi
  const handleCreateSubject = async (e) => {
    e.preventDefault();
    if (!newSubName || !newSubCode || !newSubBatch) {
      toast.error('Subject name, code aur class are required');
      return;
    }
    setIsCreating(true);
    try {
      await api.post('/subjects', {
        ...
        code: newSubCode,
        type: newSubType,
        batch: `${newSubBatch} - ${newSubSection}`,
        teacher: user._id
      });
      toast.success('Subject created successfully!');
      setSuccessMessage('✅ subject added completed!');
      setNewSubName('');
      setNewSubCode('');
      ...
      <div className="dashboard-card">
        ...
        <div style={{ marginBottom: '24px' }}>
          <button
            ...
            style={{ background: '#10b981', border: 'none', borderRadius: '8px', padding: '16px 24px', color: 'white', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}
          >
            <Calendar size={20} />
            Take Attendance
          </button>
        ...
        <div style={{ marginBottom: '24px' }}>
          <button
            ...
            style={{ background: '#3b82f6', border: 'none', borderRadius: '8px', padding: '16px 24px', color: 'white', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}
          >
            <UserPlus size={20} />
            Add User
          </button>
        ...
        {/* Add User Form */}
        {showAddStudent && (
          <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
              <div>
                ...
                <input type="text" value={newStudent.name} onChange={(e) => setNewStudent({...newStudent, name: e.target.value})} placeholder="Enter name"
                  style={{ width: '100%', padding: '8px', border: '1px solid #30363d', borderRadius: '6px', background: '#161b22', color: '#c9d1d9', fontSize: '14px' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', color: '#8b949e', fontSize: '14px' }}>Email</label>
                <input type="email" value={newStudent.email} onChange={(e) => setNewStudent({...newStudent, email: e.target.value})} placeholder="Enter email address"
                  style={{ width: '100%', padding: '8px', border: '1px solid #30363d', borderRadius: '6px', background: '#161b22', color: '#c9d1d9', fontSize: '14px' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', color: '#8b949e', fontSize: '14px' }}>Password</label>
                <input type="password" value={newStudent.password} onChange={(e) => setNewStudent({...newStudent, password: e.target.value})} placeholder="Enter password"
                  style={{ width: '100%', padding: '8px', border: '1px solid #30363d', borderRadius: '6px', background: '#161b22', color: '#c9d1d9', fontSize: '14px' }} />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={newStudent.batch} onChange={(e) => setNewStudent({...newStudent, batch: e.target.value})}
                  style={{ width: '100%', padding: '8px', border: '1px solid #30363d', borderRadius: '6px', background: '#161b22', color: '#c9d1d9', fontSize: '14px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginTop: '16px' }}>
              <button onClick={handleAddStudent} disabled={addingStudent}
                style={{ padding: '8px 20px', border: 'none', borderRadius: '6px', background: addingStudent ? '#6c757d' : '#3b82f6', color: 'white', fontSize: '14px', fontWeight: 'bold', cursor: addingStudent ? 'not-allowed' : 'pointer' }}>
                {addingStudent ? 'Adding...' : `Add ${newStudent.role === 'teacher' ? 'Teacher' : 'Student'}`}
              </button>
              <button onClick={() => { setShowAddStudent(false); setNewStudent({ name: '', email: '', password: '', batch: '', role: 'student' }); }}
                style={{ padding: '8px 20px', border: '1px solid #30363d', borderRadius: '6px', background: '#1e293b', color: '#c9d1d9', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="add-student-form" style={{ marginTop: '24px' }}>
          <div style={{ marginBottom: '14px' }}>
            <h3 style={{ color: '#e6edf3', margin: 0, fontSize: '15px', fontWeight: '600' }}>➕ Add New Subject</h3>
          </div>
        ...
        {subjects.length > 0 && (
          <div style={{ marginTop: '20px' }}>
            <h3 style={{ color: '#e6edf3', fontSize: '14px', marginBottom: '10px' }}>📚 Your Subjects:</h3>
            {subjects.map(sub => (
              <div key={sub._id}
